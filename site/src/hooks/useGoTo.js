...
export function useGoTo() {
  ...
  return (path) => {
    if (location.pathname === path) {
      ...
      // scrolls a focused element into view on its own — fighting our
      ...
      // A real mouse click on a control far down the page has the browser
      // scroll it back into view some time after this handler already ran,
      // undoing the jump above. That settles well within half a second, so
      // keep re-asserting the top for that long — whichever call runs last
      // wins.
